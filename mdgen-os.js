...
async function getApiJson() {
    const resData = await Axios
        .get("https://sg-hyp-api.hoyoverse.com/hyp/hyp-connect/api/getGamePackages?launcher_id=VYTpXlbWo8")
        .then(res => res.data);
    // 本地调试
    // const resData = FsExtra.readJsonSync('./data-os.json', { encoding: 'utf-8' });
    return resData
}
...
function _getGameConfig(biz) {
    ...
    const configMap = {
        ...
        // 崩坏：星穹铁道
        hkrpg_global: {
            mdPathGen: (m) => `global/hkrpg/Version_${m.version}.md`,
            versionTagGen: (m) => `OSRELWin${m.version}`,
        },
        // 崩三
        bh3_global: {
            mdPathGen: (m) => `global/bh3/Version_${m.version}.md`,
            versionTagGen: (m) => `OSRELWin${m.version}`,
        },
    };
...
function outputMarkdown(outputConfigs) {
    for (let outputConfig of outputConfigs) {
        ...
        // Markdown 目标绝对路径
        const mdAbsPath = Path.resolve(__dirname, './markdown', `./${mdPath}`);
